...
def main():
    ...
    total, found = print_category_stats("CHANNELING AFFILIATIONS", CHANNELING_AFFILIATIONS, categories_data)
    ...
    total, found = print_category_stats("SPECIAL ABILITIES", ability_categories, categories_data)
    all_stats["abilities"] = total

    # Nationalities
    total, found = print_category_stats("NATIONALITIES", NATIONALITY_CATEGORIES, categories_data)
    all_stats["nationalities"] = total

    # Organizations
    total, found = print_category_stats("ORGANIZATIONS", ORGANIZATIONS, categories_data)
    all_stats["organizations"] = total

    # Military Groups
    total, found = print_category_stats("MILITARY GROUPS", MILITARY_GROUPS, categories_data)
    all_stats["military_groups"] = total

    # Social Roles
    total, found = print_category_stats("SOCIAL ROLES", SOCIAL_ROLES, categories_data)
    all_stats["social_roles"] = total

    # Military Roles
    total, found = print_category_stats("MILITARY ROLES", MILITARY_ROLES, categories_data)
    all_stats["military_roles"] = total

    # Aes Sedai Positions
    total, found = print_category_stats("AES SEDAI POSITIONS", AES_SEDAI_POSITIONS, categories_data)
    all_stats["aes_sedai_positions"] = total

    # Asha'man Positions
    total, found = print_category_stats("ASHA'MAN POSITIONS", ASHAMAN_POSITIONS, categories_data)
    all_stats["ashaman_positions"] = total

    # Professions
    total, found = print_category_stats("PROFESSIONS", PROFESSIONS, categories_data)
    all_stats["professions"] = total

    # Alignment (Dark)
    total, found = print_category_stats("ALIGNMENT (DARK)", ALIGNMENT_DARK, categories_data)
    all_stats["alignment_dark"] = total

    # Cultural Groups
    total, found = print_category_stats("CULTURAL GROUPS", CULTURAL_GROUPS, categories_data)
    all_stats["cultural_groups"] = total

    # Aiel Clans
    total, found = print_category_stats("AIEL CLANS", AIEL_CLANS, categories_data)
    all_stats["aiel_clans"] = total

    # Aiel Warrior Societies
    total, found = print_category_stats("AIEL WARRIOR SOCIETIES", AIEL_SOCIETIES, categories_data)
    all_stats["aiel_societies"] = total

    # Seanchan Groups
    total, found = print_category_stats("SEANCHAN GROUPS", SEANCHAN_GROUPS, categories_data)
    all_stats["seanchan_groups"] = total

    # Atha'an Miere Groups
    total, found = print_category_stats("ATHA'AN MIERE GROUPS", ATHAAN_MIERE_GROUPS, categories_data)
