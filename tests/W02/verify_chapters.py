...
def load_config():
    ...
    return {"books_json_path": "data/raw/books", "output_report": "data/metadata/chapter_verification_report.json"}
...
def verify_chapters(books_path: str) -> dict:
    ...
    for book_file in book_files:
        try:
            ...
            # Count chapter types
            chapter_counts = {"prologue": 0, "chapter": 0, "epilogue": 0}
            ...
            for chapter in chapters:
                ...
                if len(ch_content) < 100:
                    chapter_issues.append(
                        f"Chapter {ch_num} '{ch_title}' has suspiciously short content ({len(ch_content)} chars)"
                    )
            ...
            if regular_nums != expected_nums:
                chapter_issues.append(
                    f"Chapter numbering issue: expected {expected_nums[:5]}... got {regular_nums[:5]}..."
                )
            ...
            print(
                f"   Chapters: {len(chapters):3} | P:{chapter_counts['prologue']} Ch:{chapter_counts['chapter']} E:{chapter_counts['epilogue']} | Glossary: {len(glossary)}"
            )
...
def spot_check_chapters(results: dict, num_samples: int = 20):
    ...
    for book in results["books"]:
        ...
        for ch_num in book["chapter_numbers"]:
            all_chapters.append({"book_num": book_num, "book_name": book_name, "chapter_num": ch_num})
