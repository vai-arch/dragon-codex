...
def analyze_duplicates(duplicate_groups):
    ...
    for group_name, files in duplicate_groups.items():
        ...
        for file in files:
            ...
            file_info.append(
                {"path": file, "name": file.name, "size": size, "has_duplicate_suffix": has_duplicate_suffix}
            )
        ...
        if largest["has_duplicate_suffix"]:
            ...
            rename_action = {"from": largest["path"], "to": new_path}

        actions.append({"group": group_name, "keep": largest, "delete": to_delete, "rename": rename_action})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--confirm", action="store_true", help="Ask before performing the cleanup")
