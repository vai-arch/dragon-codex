...
def extract_non_temporal_sections(content):
    ...
    for match in all_h2:
        ...
        non_temporal.append(
            {"title": section_title, "content_length": len(section_content), "preview": section_content[:200].strip()}
        )
...
def test_wiki_parsing(wiki_path):
    ...
    if temporal:
        ...
        for section in temporal:
            ...
            print(f"           {section['content_length']:,} chars, {section['subsection_count']} subsections")
    ...
    return {
        ...
        "books_appeared": [map_book_title_to_number(t["book_title"]) for t in temporal],
    }
...
def main():
    ...
    test_characters = ["Rand_al'Thor", "Egwene_al'Vere", "Moiraine_Damodred", "Perrin_Aybara", "Mat_Cauthon"]
    ...
    if results:
        ...
        for r in results:
            ...
            print(
                f"    Appears in {r['temporal_sections']} books: {sorted([b for b in r['books_appeared'] if b >= 0])}"
            )
