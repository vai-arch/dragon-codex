...
def create_narrative_collection(client: chromadb.Client, config, reset: bool = False):
    ...
    collection = client.create_collection(
        name=collection_name, metadata={"description": "Book narrative chunks with character evolution"}
    )
...
def create_concepts_collection(client: chromadb.Client, config, reset: bool = False):
    ...
    print(f"\nCreating collection: {collection_name}")
    collection = client.create_collection(
        name=collection_name, metadata={"description": "Wiki chunks for WoT lore and definitions"}
    )
...
def create_magic_collection(client: chromadb.Client, config, reset: bool = False):
    ...
    print(f"\nCreating collection: {collection_name}")
    collection = client.create_collection(
        name=collection_name, metadata={"description": "Chunks with magic system mentions"}
    )
...
def test_persistence(client: chromadb.Client, config):
    ...
    new_client = chromadb.PersistentClient(
        path=str(config.VECTOR_STORE_PATH), settings=Settings(anonymized_telemetry=config.CHROMA_TELEMETRY)
    )
...
def main(collection: str = None, reset: bool = False):
    ...
    client = chromadb.PersistentClient(
        path=str(persist_directory), settings=Settings(anonymized_telemetry=config.CHROMA_TELEMETRY)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--collection", choices=["narrative", "concepts", "magic"], help="Create specific collection only"
    )
