...
class VectorStoreManager:
    ...
    def test_connection(self) -> bool:
        ...
        try:
            response = requests.post(
                f"{self.ollama_url}/api/embeddings", json={"model": self.model, "prompt": "test"}, timeout=10
            )
    ...
    def embed_batch(self, texts: List[str], batch_size: int = 100) -> Tuple[List[List[float]], int]:
        ...
        for i in pbar:
            ...
            response = self.session.post(
                f"{self.ollama_url}/api/embed", json={"model": self.model, "input": batch_texts}
            )
            response.raise_for_status()
            data = response.json()
            ...
            pbar.set_postfix_str(f"Procc Items: {total_items} | AVG Tokens: {running_avg_tokens:.1f}")

        return all_embeddings, max_tokens

    def embed_batch_parallel(
        self, texts: List[str], batch_size: int = 32, max_workers: int = 4
        ...
        def process_batch(batch_texts):
            ...
            response = self.session.post(
                f"{self.ollama_url}/api/embed", json={"model": self.model, "input": batch_texts}, timeout=300
            )
            response.raise_for_status()
            data = response.json()
    ...
    def keyword_search(self, query_text: str, collection_name: str, k: int = 10) -> List[Dict]:
        ...
        for i, doc in enumerate(results["documents"]):
            ...
            if score > 0:
                scored.append({"text": doc, "metadata": results["metadatas"][i], "keyword_score": score})
    ...
    def hybrid_query(
        self, query_text: str, collection_name: str, k: int = 10, max_book: Optional[int] = None
    ) -> List[Dict]:
    ...
    def query(self, query_text: str, collection_name: str, k: int = 10, max_book: Optional[int] = None) -> List[Dict]:
