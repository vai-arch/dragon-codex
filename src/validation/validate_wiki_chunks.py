...
def validate_chunks():
    ...
    for source_name, chunks in all_chunks.items():
        for i, chunk in enumerate(chunks):
            ...
            if missing:
                issues.append(
                    {"type": "missing_metadata", "source": source_name, "chunk_index": i, "missing_fields": missing}
                )
    ...
    for source_name, chunks in all_chunks.items():
        for i, chunk in enumerate(chunks):
            ...
            if text_len == 0:
                issues.append({"type": "empty_text", "source": source_name, "chunk_index": i})
    ...
    for source_name, stats in size_stats.items():
        print(
            f"   {source_name:25} | {stats['count']:6,} chunks | "
            ...
            json.dump(
                {
                    ...
                    "temporal_issues": [{"source": s, "index": i, "value": t} for s, i, t in invalid_temporal],
                },
                f,
                indent=2,
            )
