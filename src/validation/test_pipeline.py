...
class PipelineValidator:
    ...
    def validate_chunk_file(self, file_name: str, file_path: Path):
        ...
        print(f"\n   📊 Enrichment Coverage:")
        print(
            f"      Characters: {stats['with_character_mentions']:6,} chunks ({char_pct:5.1f}%) - {stats['total_characters']:,} mentions"
        )
        print(
            f"      Concepts:   {stats['with_concept_mentions']:6,} chunks ({concept_pct:5.1f}%) - {stats['total_concepts']:,} mentions"
        )
        print(
            f"      Magic:      {stats['with_magic_mentions']:6,} chunks ({magic_pct:5.1f}%) - {stats['total_magic']:,} mentions"
        )
        print(
            f"      Prophecies: {stats['with_prophecy_mentions']:6,} chunks ({prophecy_pct:5.1f}%) - {stats['total_prophecies']:,} mentions"
        )
    ...
    def print_summary(self, all_stats: dict):
        ...
        print(
            f"   With concept mentions:      {total_with_concepts:,} ({total_with_concepts / total_chunks * 100:.1f}%)"
        )
        print(f"   With magic mentions:        {total_with_magic:,} ({total_with_magic / total_chunks * 100:.1f}%)")
        print(
            f"   With prophecy mentions:     {total_with_prophecies:,} ({total_with_prophecies / total_chunks * 100:.1f}%)"
        )
        ...
        for file_name, stats in all_stats.items():
            ...
            print(
                f"{file_name:20} {stats['total']:8,} {char_pct:7.1f}% {concept_pct:8.1f}% {magic_pct:7.1f}% {prophecy_pct:9.1f}%"
            )
