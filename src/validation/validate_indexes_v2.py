...
EXPECTED_MAGIC_PAGES = ["One Power", "Angreal", "Sa'angreal", "Ter'angreal", "Saidin", "Channeling"]

# Expected prophecy pages
EXPECTED_PROPHECY_PAGES = ["Karaethon Cycle", "Foretelling", "Min's viewings"]
...
class IndexValidator:
    ...
    def validate_character_index(self):
        ...
        print(
            f"   With gender:                {stats['with_gender']:6,} ({stats['with_gender'] / stats['total'] * 100:5.1f}%)"
        )
        print(
            f"   With channeling:            {stats['with_channeling']:6,} ({stats['with_channeling'] / stats['total'] * 100:5.1f}%)"
        )
        print(
            f"   With aliases:               {stats['with_aliases']:6,} ({stats['with_aliases'] / stats['total'] * 100:5.1f}%)"
        )
        print(
            f"   With nationalities:         {stats['with_nationalities']:6,} ({stats['with_nationalities'] / stats['total'] * 100:5.1f}%)"
        )
        print(
            f"   With special abilities:     {stats['with_special_abilities']:6,} ({stats['with_special_abilities'] / stats['total'] * 100:5.1f}%)"
        )
    ...
    def validate_magic_index(self):
        ...
        print(
            f"   With descriptions:          {stats['with_description']:6,} ({stats['with_description'] / stats['total'] * 100:5.1f}%)"
        )
        ...
        for page_name in EXPECTED_MAGIC_PAGES:
            ...
            if page.get("description"):
                desc_preview = (
                    page["description"][:80] + "..." if len(page["description"]) > 80 else page["description"]
                )
