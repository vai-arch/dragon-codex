...
class ChunkValidator:
    ...
    def validate_common_fields(self, chunk: Dict, chunk_index: int, source_name: str) -> List[str]:
        ...
        for field, expected_type in required_fields.items():
            if field not in chunk:
                ...
                errors.append(
                    f"Field '{field}' has wrong type: expected {expected_type.__name__}, got {type(chunk[field]).__name__}"
                )
    ...
    def validate_chunk_file(self, file_path: Path, chunk_type: str):
        ...
        for i, chunk in enumerate(chunks):
            ...
            if errors:
                chunk_errors += 1
                self.issues.append({"file": chunk_type, "chunk_index": i, "errors": errors})
...
"""
    ...
    def generate_statistics_report(self):
        ...
        for source_name, stats in self.stats.items():
            ...
            report.append(
                f"{source_name:<25} {stats['total_chunks']:8,} "
        ...
        report.append(
            f"Temporal chunks (with book_number): {total_temporal:,} ({total_temporal / total_chunks * 100:.1f}%)"
        )
        report.append(
            f"Non-temporal chunks (reference): {total_non_temporal:,} ({total_non_temporal / total_chunks * 100:.1f}%)"
        )
        report.append("")
